...
//        graphics.fill(x, y, x + width, y + height, bgColor);
        ...
        if (icon != null) {
            if (imagePosition == ImagePosition.IMAGE_LEFT) {
                ...
                contentX += iconWidth + (label != null && !text.isEmpty() ? spacing : 0);
            }
        }

        if (label != null && !text.isEmpty()) {
            SingletonInstances.DRAWING_UTILS.drawScaledText(graphics, text, contentX, y + (height - font.lineHeight() * fontScale) / 2, textColor, fontScale, true);
            if (icon != null && imagePosition == ImagePosition.IMAGE_RIGHT_KINDA) {
                contentX += (int) (font.width(text) * fontScale) + spacing;
                graphics.blit(icon, contentX, contentY, 0, 0, iconWidth, iconHeight, iconWidth, iconHeight);
            }
